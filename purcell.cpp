...
Decay_rates Purcell(double k0, /* Computing the rad and non rad decay rates by perfoming the kp integral*/
                    ...
                    double z0 , 
                    std::string dir, 
                    size_t limit = 128,size_t m_deg = 10){  // sets (2m+1)-point Gauss-Kronrod
    ...
    return Decay_rates { 1+ 1.5f*(rad + non_rad), rad, non_rad, err_r/rad, err_nr/non_rad}; // returns , total decay rate, rad decay, non rad decay, numerical integration erros for rad and non rad
